class SubProblemSolver
	...
	@@array_for_dp = Array.new(2) { Array.new(2, nil) }
	#ビル1階、卵1個の時のDP用配列を作っておく
	...
	def initialize(number_of_floor, number_of_eggs)
		...
		if @@array_for_dp.length < number_of_floor + 1
			(1..(number_of_floor + 1 - @@array_for_dp.length)).each do |e|
				@@array_for_dp.push(Array.new(number_of_eggs + 1, nil))
			end
		end
		...
		if @number_of_eggs == 1
			...
			exlopred_floors.each do |trial_floor|
				if @@array_for_dp[@number_of_floor - trial_floor][@number_of_eggs].nil?
					 sub_problem_in_case_of_not_crash = SubProblemSolver.new(@number_of_floor - trial_floor, @number_of_eggs)
					 @@array_for_dp[@number_of_floor - trial_floor][@number_of_eggs] = sub_problem_in_case_of_not_crash.find_optimal_value
				...
				if @@array_for_dp[trial_floor - 1][@number_of_eggs - 1].nil?
					sub_problem_in_case_of_crash = SubProblemSolver.new(trial_floor - 1, @number_of_eggs - 1)
					@@array_for_dp[trial_floor - 1][@number_of_eggs - 1] = worst_number_of_trials_in_case_of_crash = sub_problem_in_case_of_crash.find_optimal_value
